/*
 * Preneto iz @fusionstrings/swisseph-wasi 0.1.0 (AGPL-3.0). Ovaj repozitorijum je
 * takodje AGPL-3.0, pa je prenosenje dozvoljeno; vidi NOTICE.
 *
 * Zasto preneto umesto uvezeno:
 *  - paket kroz Deno-shim sloj vuce `undici`, `which` i `path`, kojih u Workers-u nema;
 *    `nodejs_compat` bi ih ubacio u paket bez ikakve koristi,
 *  - u 0.1.0 `load()` trazi WASM na putanji na kojoj ga nema (greska u pakovanju).
 *
 * Izmene, sve popisane:
 *  1. uklonjen uvoz Deno-shim-a,
 *  2. uklonjena funkcija load(),
 *  3. putanja do generisanog API-ja pokazuje na ./api.js.
 */
import { WASI } from "./wasi.js";
import { WasmHeap } from "./heap.js";
...
export class SwissEph {
    constructor(module) {
        ...
        Object.defineProperty(this, "wasi", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        ...
        this.wasi.setMemory(this.instance.exports.memory);
        this.exports = this.instance.exports;
        this.heap = new WasmHeap(this.instance.exports.memory, this.exports);
    }
    /**
     ...
     */
    swe_calc(tjd_et, ipl, iflag, xx_ptr, serr_ptr) {
    ...
    swe_revjul(jd, gregflag) {
        ...
        this.exports.swe_revjul(jd, gregflag, year_ptr, month_ptr, day_ptr, hour_ptr);
    ...
    swe_utc_to_jd(year, month, day, hour, min, sec, gregflag) {
        ...
        const serr_ptr = this.heap.alloc(256);
        const ret = this.exports.swe_utc_to_jd(year, month, day, hour, min, sec, gregflag, dret_ptr, serr_ptr);
     ...
     */
    swe_jdet_to_utc(tjd_et, gregflag) {
        const year_ptr = this.heap.alloc(4);
        const month_ptr = this.heap.alloc(4);
        const day_ptr = this.heap.alloc(4);
        ...
        this.exports.swe_jdet_to_utc(tjd_et, gregflag, year_ptr, month_ptr, day_ptr, hour_ptr, min_ptr, sec_ptr);
     ...
     * @returns Object with year, month, day, hour, min, sec
     */
    swe_jdut1_to_utc(tjd_ut, gregflag) {
        const year_ptr = this.heap.alloc(4);
        const month_ptr = this.heap.alloc(4);
        const day_ptr = this.heap.alloc(4);
        const hour_ptr = this.heap.alloc(4);
        const min_ptr = this.heap.alloc(4);
        const sec_ptr = this.heap.alloc(8);
        this.exports.swe_jdut1_to_utc(tjd_ut, gregflag, year_ptr, month_ptr, day_ptr, hour_ptr, min_ptr, sec_ptr);
        const view = new DataView(this.heap.getU8(year_ptr, 20).buffer);
        const year = view.getInt32(0, true);
        const month = view.getInt32(4, true);
        const day = view.getInt32(8, true);
    ...
    swe_house_pos(armc, geolat, eps, hsys, xpin) {
        ...
        const pos = this.exports.swe_house_pos(armc, geolat, eps, hsys, xpin_ptr, serr_ptr);
        const error = this.heap.getString(serr_ptr);
     ...
     */
    swe_azalt_rev(tjd_ut, calc_flag, geopos, xin) {
        const geopos_ptr = this.heap.alloc(24);
     ...
     * @returns Object containing status code, position array `xx`, star name (resolved), and error string
     */
    swe_fixstar2_ut(star, tjd_ut, iflag) {
        const star_ptr = this.heap.alloc(256);
        const xx_ptr = this.heap.alloc(48);
        const serr_ptr = this.heap.alloc(256);
        const starBytes = new TextEncoder().encode(star + "\0");
    ...
    swe_sol_eclipse_when_glob(tjd_start, ifl, ifltype, backward) {
        ...
        const serr_ptr = this.heap.alloc(256);
        const ret = this.exports.swe_sol_eclipse_when_glob(tjd_start, ifl, ifltype, tret_ptr, backward ? 1 : 0, serr_ptr);
     ...
     */
    swe_lun_eclipse_when(tjd_start, ifl, ifltype, backward) {
        const tret_ptr = this.heap.alloc(80);
        const serr_ptr = this.heap.alloc(256);
        const ret = this.exports.swe_lun_eclipse_when(tjd_start, ifl, ifltype, tret_ptr, backward ? 1 : 0, serr_ptr);
        const tret = this.heap.getF64(tret_ptr, 10).slice();
        const error = this.heap.getString(serr_ptr);
        this.heap.free(tret_ptr);
        this.heap.free(serr_ptr);
     ...
     */
    swe_nod_aps(tjd_et, ipl, iflag, method) {
     ...
     */
    swe_get_orbital_elements(tjd_et, ipl, iflag) {
     ...
     */
    swe_heliacal_ut(tjdstart_ut, geopos, datm, dobs, ObjectName, TypeEvent, iflag) {
     ...
     */
    swe_heliacal_pheno_ut(tjd_ut, geopos, datm, dobs, ObjectName, TypeEvent, helflag) {
        const geopos_ptr = this.heap.alloc(3 * 8);
        this.heap.setU8(geopos_ptr, new Uint8Array(new Float64Array(geopos).buffer));
        const datm_ptr = this.heap.alloc(6 * 8);
        this.heap.setU8(datm_ptr, new Uint8Array(new Float64Array(datm).buffer));
     ...
     * swe_vis_limit_mag
     */
    swe_vis_limit_mag(tjdut, geopos, datm, dobs, ObjectName, helflag) {
        const geopos_ptr = this.heap.alloc(3 * 8);
        this.heap.setU8(geopos_ptr, new Uint8Array(new Float64Array(geopos).buffer));
        const datm_ptr = this.heap.alloc(6 * 8);
        this.heap.setU8(datm_ptr, new Uint8Array(new Float64Array(datm).buffer));
     ...
     * swe_heliacal_angle
     */
    swe_heliacal_angle(tjdut, datm, dobs, helflag, mag, azi_obj, azi_sun, azi_moon, alt_moon) {
     ...
     */
    swe_topo_arcus_visionis(tjdut, datm, dobs, helflag, mag, azi_obj, alt_obj, azi_sun, azi_moon, alt_moon) {
        const dgeo_ptr = this.heap.alloc(6 * 8);
        const datm_ptr = this.heap.alloc(6 * 8);
        this.heap.setU8(datm_ptr, new Uint8Array(new Float64Array(datm).buffer));
        const dobs_ptr = this.heap.alloc(6 * 8);
     ...
     */
    swe_get_astro_models(samod, sdet, iflag) {
        const samod_ptr = this.heap.putString(samod);
     ...
     * swe_calc_pctr
     */
    swe_calc_pctr(tjd, ipl, iplctr, iflag) {
     ...
     * swe_solcross
     */
    swe_solcross(x2cross, jd_et, flag) {
        const serr_ptr = this.heap.alloc(256);
     ...
     * swe_solcross_ut
     */
    swe_solcross_ut(x2cross, jd_ut, flag) {
        const serr_ptr = this.heap.alloc(256);
     ...
     * swe_mooncross
     */
    swe_mooncross(x2cross, jd_et, flag) {
        const serr_ptr = this.heap.alloc(256);
     ...
     */
    swe_mooncross_ut(x2cross, jd_ut, flag) {
        const serr_ptr = this.heap.alloc(256);
     ...
     * @returns Object containing node positions and error string
     */
    swe_mooncross_node_ut(jd_ut, flag) {
        const xlon_ptr = this.heap.alloc(6 * 8);
        const xlat_ptr = this.heap.alloc(6 * 8);
        const serr_ptr = this.heap.alloc(256);
     ...
     * @returns Object containing position, status code, and error string
     */
    swe_fixstar_ut(star, tjd_ut, iflag) {
        const star_ptr = this.heap.putString(star);
        const xx_ptr = this.heap.alloc(6 * 8);
        const serr_ptr = this.heap.alloc(256);
     ...
     */
    swe_date_conversion(y, m, d, year, arg4, c) {
    ...
    swe_houses_ex2(tjd_ut, iflag, geolat, geolon, hsys) {
        ...
        const ascmc_speed = Array.from(this.heap.getF64(ascmc_speed_ptr, 10).slice());
        const serr = this.heap.getString(serr_ptr);
        this.heap.free(cusps_ptr);
        this.heap.free(ascmc_ptr);
     ...
     */
    swe_gauquelin_sector(t_ut, ipl, starname, iflag, imeth, geopos, atpress, attemp) {
     ...
     */
    swe_sol_eclipse_where(tjd, ifl, geopos) {
        const geopos_ptr = this.heap.alloc(3 * 8);
        this.heap.setU8(geopos_ptr, new Uint8Array(new Float64Array(geopos).buffer));
     ...
     */
    swe_lun_occult_where(tjd, ipl, starname, ifl, geopos) {
        const starname_ptr = this.heap.putString(starname);
        const geopos_ptr = this.heap.alloc(3 * 8);
        this.heap.setU8(geopos_ptr, new Uint8Array(new Float64Array(geopos).buffer));
        const attr_ptr = this.heap.alloc(20 * 8);
     ...
     * @returns Object containing eclipse attributes and error string
     */
    swe_sol_eclipse_how(tjd, ifl, geopos) {
        const geopos_ptr = this.heap.alloc(3 * 8);
        this.heap.setU8(geopos_ptr, new Uint8Array(new Float64Array(geopos).buffer));
        const attr_ptr = this.heap.alloc(20 * 8);
        const serr_ptr = this.heap.alloc(256);
     ...
     */
    swe_sol_eclipse_when_loc(tjd_start, ifl, geopos, backward) {
        const geopos_ptr = this.heap.alloc(3 * 8);
        this.heap.setU8(geopos_ptr, new Uint8Array(new Float64Array(geopos).buffer));
     ...
     * @returns Object containing result array `tret`, attributes, and error string
     */
    swe_lun_occult_when_loc(tjd_start, ipl, starname, ifl, geopos, backward) {
        const starname_ptr = this.heap.putString(starname);
        const geopos_ptr = this.heap.alloc(3 * 8);
        this.heap.setU8(geopos_ptr, new Uint8Array(new Float64Array(geopos).buffer));
        const tret_ptr = this.heap.alloc(10 * 8);
     ...
     */
    swe_lun_occult_when_glob(tjd_start, ipl, starname, ifl, ifltype, backward) {
        const starname_ptr = this.heap.putString(starname);
        const tret_ptr = this.heap.alloc(10 * 8);
        const serr_ptr = this.heap.alloc(256);
     ...
     * @returns Object containing eclipse attributes and error string
     */
    swe_lun_eclipse_how(tjd_ut, ifl, geopos) {
        const geopos_ptr = this.heap.alloc(3 * 8);
        this.heap.setU8(geopos_ptr, new Uint8Array(new Float64Array(geopos).buffer));
        const attr_ptr = this.heap.alloc(20 * 8);
        const serr_ptr = this.heap.alloc(256);
     ...
     * @returns Object containing result array `tret`, attributes, and error string
     */
    swe_lun_eclipse_when_loc(tjd_start, ifl, geopos, backward) {
        const geopos_ptr = this.heap.alloc(3 * 8);
        this.heap.setU8(geopos_ptr, new Uint8Array(new Float64Array(geopos).buffer));
        const tret_ptr = this.heap.alloc(10 * 8);
        const attr_ptr = this.heap.alloc(20 * 8);
     ...
     */
    swe_refrac_extended(inalt, geoalt, atpress, attemp, lapse_rate, calc_flag) {
        const dret_ptr = this.heap.alloc(6 * 8);
     ...
     */
    swe_orbit_max_min_true_distance(tjd_et, ipl, iflag) {
     ...
     * swe_cs2timestr
     */
    swe_cs2timestr(t, sep, suppressZero, a) {
     ...
     * swe_cs2lonlatstr
     */
    swe_cs2lonlatstr(t, pchar, mchar, s) {
 ...
 */
// load() uklonjen: citao je WASM sa diska preko Deno/fetch. U Workers-u se WASM
// uvozi kao modul i predaje direktno konstruktoru SwissEph.
